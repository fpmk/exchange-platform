// import { WalletPort } from '@exchange-platform/ports';
// import {
//   catchError,
//   combineLatest,
//   map,
//   Observable,
//   of,
//   Subject,
//   switchMap,
//   tap,
//   throwError,
// } from 'rxjs';
// import { Wallet, WalletAccount } from '@exchange-platform/wallet';
// import { ChainId, ProviderId, WalletAddress } from '@exchange-platform/types';
// import { inject, Injectable } from '@angular/core';
// import { WINDOW } from '@exchange-platform/platform';
// import {
//   BrowserProvider,
//   Eip1193Provider,
//   ethers,
//   JsonRpcSigner,
// } from 'ethers';
// import { fromPromise } from 'rxjs/internal/observable/innerFrom';
// import { WalletProvidersService } from '@exchange-platform/application-services';
//
// @Injectable({ providedIn: 'root' })
// export class MetaMaskAdapter implements WalletPort {
//   readonly providerId: ProviderId = 'metamask';
//   readonly name = 'MetaMask';
//   readonly icon =
//     'data:image/svg+xml;base64,';
//   readonly installUrl = 'https://metamask.io/download/';
//   readonly priority = 100;
//   readonly supportedChains = [ChainId.ETHEREUM_MAINNET];
//   private readonly _window = inject(WINDOW);
//
//   private wallet!: Wallet;
//   private address!: WalletAddress;
//   private provider!: BrowserProvider;
//   private signer!: JsonRpcSigner;
//   private alreadyRegistered = false;
//   private accountsChangedSubject = new Subject<string>();
//
//   isAvailable(): boolean {
//     return !!this._window.ethereum?.isMetaMask;
//   }
//
//   connect(): Observable<Wallet> {
//     if (!this.isAvailable()) {
//       throw new Error('Install MetaMask wallet');
//     }
//     return fromPromise(
//       this._window.ethereum?.request({
//         method: 'wallet_requestPermissions',
//         params: [{ eth_accounts: {} }],
//       }) as Promise<unknown>
//     ).pipe(
//       switchMap((res) => {
//         console.log(res);
//         return fromPromise(
//           this._window.ethereum?.request({
//             method: 'eth_accounts',
//           }) as Promise<any>
//         );
//       }),
//       tap(() => this.initializeEventListeners()),
//       tap(
//         () =>
//           (this.provider = new ethers.BrowserProvider(
//             this._window.ethereum as Eip1193Provider
//           ))
//       ),
//       switchMap((accounts) => {
//         console.log(accounts);
//         return this.createWallet(accounts[0]);
//       })
//     );
//     // return of(this.createWallet(accounts[0]));
//     // return fromPromise(this.provider.getSigner()).pipe(
//     //   switchMap((signer) => {
//     //     this.signer = signer;
//     //     return fromPromise(signer.getAddress());
//     //   }),
//     //   switchMap((address) => this.createWallet(address)),
//     //   tap(() => this.initializeEventListeners())
//     // );
//   }
//
//   disconnect(): Observable<void> {
//     return of(undefined);
//   }
//
//   getAccount(): Observable<WalletAccount> {
//     return fromPromise(this.signer.getAddress()).pipe(
//       switchMap((account) => this.mapAccount(account)),
//       catchError((error: Error) => throwError(() => error))
//     );
//   }
//
//   signMessage(message: string): Observable<string> {
//     if (!this.address) {
//       return throwError(() => new Error('No account selected'));
//     }
//     return fromPromise(this.signer.signMessage(message));
//   }
//
//   onAccountChanged(): Observable<string> {
//     return this.accountsChangedSubject.asObservable();
//   }
//
//   private mapAccount(account: string): Observable<WalletAccount> {
//     return combineLatest([
//       fromPromise(this.provider.getBalance(account)),
//       this.getCurrentChain(),
//     ]).pipe(
//       map(([balance, chainId]) => {
//         return {
//           provider: this.wallet,
//           address: account as WalletAddress,
//           balance: balance.toString(),
//           chainId: chainId,
//         } as WalletAccount;
//       })
//     );
//   }
//
//   private createWallet(account: string): Observable<Wallet> {
//     return combineLatest([
//       fromPromise(this.getCurrentChain()),
//       this.mapAccount(account),
//     ]).pipe(
//       map(([chainId, account]) => {
//         this.wallet = {
//           provider: {
//             id: this.providerId,
//             name: this.name,
//             icon: this.icon,
//             installUrl: this.installUrl,
//             priority: this.priority,
//             supportedChains: this.supportedChains,
//           },
//           isConnected: !!account?.address,
//           account,
//           chainId,
//         };
//         return this.wallet;
//       })
//     );
//   }
//
//   private async getCurrentChain(): Promise<ChainId> {
//     const network = await this.provider.getNetwork();
//     return (Number(network.chainId) as ChainId) || ChainId.ETHEREUM_MAINNET;
//   }
//
//   private initializeEventListeners(): void {
//     if (this.alreadyRegistered) return;
//     const { ethereum } = window;
//     if (ethereum && ethereum.on) {
//       ethereum.on('accountsChanged', (accounts: string[]) => {
//         console.log(`${this.providerId}: Accounts changed`, accounts);
//         this.provider = new ethers.BrowserProvider(
//           this._window.ethereum as Eip1193Provider
//         );
//         this.accountsChangedSubject.next(accounts[0]);
//       });
//       this.alreadyRegistered = true;
//     }
//   }
// }
